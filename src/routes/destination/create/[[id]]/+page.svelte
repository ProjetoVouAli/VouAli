<script lang="ts">
	...
	let searchQuery = $derived.by(() => {
		// Don't auto-geocode after user manually placed the pin
	...
	let allStates = $derived(
		[...new Set(data.cities?.map((c: any) => c.state)?.filter(Boolean) ?? [])].sort()
	);
	...
	// Sync schedule JSON into form data
	$effect(() => {
		$formData.scheduleData = scheduleJson;
	});
	
	// Enable schedule toggle if there are existing slots
	$effect(() => {
		if (data.existingSlots && data.existingSlots.length > 0) {
	...
	function addSlot() {
		scheduleSlots = [...scheduleSlots, { dayOfWeek: 'MON', startTime: '08:00', endTime: '18:00' }];
	}
	
	function removeSlot(idx: number) {
		scheduleSlots = scheduleSlots.filter((_, i) => i !== idx);
	}
</script>
...
<div class="container max-w-4xl mx-auto py-10 px-4">
	...
	>
		...
		<div class="space-y-4">
			...
			{#if geoStatus && geoStatus !== '✓' && !geoStatus.startsWith('✓')}
				<p class="text-xs text-muted-foreground bg-muted px-3 py-1 rounded inline-block">
					{geoStatus}
				</p>
			{/if}
		...
		<div class="space-y-4">
			<div class="flex items-center gap-3">
				<input type="checkbox" id="scheduleToggle" bind:checked={$formData.scheduleEnabled}
					class="w-5 h-5 accent-primary cursor-pointer" />
				<label for="scheduleToggle" class="text-lg font-semibold cursor-pointer">Definir horários de funcionamento</label>
			</div>
			...
			{#if $formData.scheduleEnabled}
				<div class="space-y-3 pl-8">
					{#each scheduleSlots as slot, idx (idx)}
						<div class="flex flex-wrap gap-3 items-end p-3 bg-muted/20 rounded-lg border border-border">
							<div>
								...
								<select bind:value={slot.dayOfWeek} class="px-3 py-2 border-2 border-primary bg-background text-foreground rounded-md text-sm">
							...
							<div>
								<label class="block text-xs font-semibold mb-1">Início</label>
								<input type="time" bind:value={slot.startTime} class="px-3 py-2 border-2 border-primary bg-background text-foreground rounded-md text-sm" />
							</div>
							<div>
								<label class="block text-xs font-semibold mb-1">Fim</label>
								<input type="time" bind:value={slot.endTime} class="px-3 py-2 border-2 border-primary bg-background text-foreground rounded-md text-sm" />
							</div>
							<button type="button" onclick={() => removeSlot(idx)}
								class="px-3 py-2 text-sm font-bold text-destructive border-2 border-destructive hover:bg-destructive hover:text-destructive-foreground transition-colors rounded-md">
								Remover
							</button>
						</div>
					{/each}
					...
					<button type="button" onclick={addSlot}
						class="px-4 py-2 text-sm font-bold border-2 border-primary text-foreground hover:bg-primary hover:text-primary-foreground transition-colors rounded-md">
						+ Adicionar Horário
					</button>
				</div>
			{/if}
