  ...
  <div class="max-w-3xl mx-auto px-8">
        ...
        <CardContent>
          <form method="POST" action="?/updateProfile" use:enhance enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
              ...
              <textarea id="bio" name="bio" class="w-full px-3 py-2 border-2 border-primary bg-background text-foreground rounded-md focus:outline-none focus:ring-2 focus:ring-ring" rows="4">{data.user?.bio || ''}</textarea>
            </div>
            <div>
              <label for="avatar" class="block text-sm font-semibold mb-2">Foto de Perfil</label>
              <input id="avatar" type="file" name="avatar" accept="image/*" class="w-full text-sm file:mr-4 file:py-2 file:px-4 file:border-2 file:border-primary file:bg-background file:text-foreground file:font-semibold file:cursor-pointer hover:file:bg-accent" />
              {#if data.user?.avatarUrl}
                <img src={data.user.avatarUrl} alt="Avatar atual" class="mt-2 w-16 h-16 rounded-full object-cover border border-border" />
              {/if}
            </div>
    ...
    {#if activeTab === 'security'}
      <Card>
        ...
        <CardContent>
          <p class="text-muted-foreground mb-4">Um link de redefinição será enviado para seu e-mail cadastrado ({data.user?.email}).</p>
